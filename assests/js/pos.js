...
function gettingCartItems() {
  ...
  let cartItems = [];
  trs.map((tr) => {
    console.log(tr.querySelector(".food-qty input").value);
    const item = {
      id: tr.id,
      varient: tr.querySelector(".varient")?.textContent,
      qty: tr.querySelector(".food-qty input").value,
    };
...
placeOrderBtn.addEventListener("click", () => {
  ...
  var totalAmount = grandInput.value;

  const productInfo = JSON.stringify(gettingCartItems());
...
ipcRenderer.on("ItemsOnBasketSent", (e, items) => {
  ...
  items.map((x) => {
    ...
    addOnItems.map((addOnItem) => {
      if (addOnItem.addOnName) {
        newAddons.push(addOnItem.addOnName);
      }

      console.log(newAddons.indexOf(addOnItem.addOnName));
...
function updateCartTotal() {
  ...
  for (var i = 0; i < cartRows.length; i++) {
    ...
    const priceElement = cartRow.getElementsByClassName("foodTotal")[0];
...
function checkProductInfo() {
  ...
  const allTrFromCart = "Khabar dabar";

  return allTrFromCart;
}
...
ipcRenderer.on("foodsByCategoryReplySent", (evt, foods) => {
  ...
  foods.forEach((food) => {
    ...
    <img src="${baseURL}${food.ProductImage}" height="100" width="206" class="card-img-top">
      <div class="food_items" style="text-align: center;"><p><a href="#"  style="text-decoration:none; color:black;" id=${food.ProductsID} onclick = {getFoodId(${food.ProductsID})}>
      ${food.ProductName}
        </a></p>
      </div>
      </div>
