...
ipcMain.on("foodIdSent", (evt, result) => {
  ...
  const cartItems = `SELECT DISTINCT FoodList.ProductName, VarientList.variantName, VarientList.price, FoodList.ProductsID
  ...
  const addonItems = `SELECT  DISTINCT AddsOn.add_on_name, AddsOn.price, FoodList.ProductsID
...
ipcMain.on("foodTable", () => {
  ...
  const foodTable = `	SELECT Tables.tablename, Tables.customer_name, Tables.table_icon
  FROM Tables`;
